using RecruitmentPlatformAPI.DTOs;
using RecruitmentPlatformAPI.DTOs.Auth;
using RecruitmentPlatformAPI.DTOs.Profile;
using RecruitmentPlatformAPI.DTOs.Reference;
using RecruitmentPlatformAPI.Models.Core;
using RecruitmentPlatformAPI.Models.Reference;
using RecruitmentPlatformAPI.Models.Authentication;
using RecruitmentPlatformAPI.Models.Assessment;
using RecruitmentPlatformAPI.Services;
using RecruitmentPlatformAPI.Services.Interfaces;
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using RecruitmentPlatformAPI.Data;

namespace RecruitmentPlatformAPI.Data.Migrations {
[DbContext(typeof(AppDbContext))]
    [Migration("20260108035219_UpdateJobTitleRoleFamilySeeds")]
    partial class UpdateJobTitleRoleFamilySeeds
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.AssessmentAnswer", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("AnsweredAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("AssessmentAttemptId")
                        .HasColumnType("int");

                    b.Property<bool>("IsCorrect")
                        .HasColumnType("bit");

                    b.Property<int>("QuestionId")
                        .HasColumnType("int");

                    b.Property<int>("SelectedAnswerIndex")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("QuestionId");

                    b.HasIndex("AssessmentAttemptId", "QuestionId")
                        .IsUnique()
                        .HasDatabaseName("IX_AssessmentAnswer_Attempt_Question");

                    b.ToTable("AssessmentAnswer", (string)null);
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.AssessmentAttempt", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime?>("CompletedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<int>("JobSeekerId")
                        .HasColumnType("int");

                    b.Property<int>("JobTitleId")
                        .HasColumnType("int");

                    b.Property<decimal?>("OverallScore")
                        .HasPrecision(5, 2)
                        .HasColumnType("decimal(5,2)");

                    b.Property<DateTime?>("ScoreExpiresAt")
                        .HasColumnType("datetime2");

                    b.Property<decimal?>("SoftSkillsScore")
                        .HasPrecision(5, 2)
                        .HasColumnType("decimal(5,2)");

                    b.Property<DateTime>("StartedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<decimal?>("TechnicalScore")
                        .HasPrecision(5, 2)
                        .HasColumnType("decimal(5,2)");

                    b.Property<int>("TimeLimitMinutes")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("JobTitleId");

                    b.HasIndex("JobSeekerId", "IsActive")
                        .HasDatabaseName("IX_AssessmentAttempt_JobSeeker_Active");

                    b.HasIndex("JobSeekerId", "Status", "StartedAt")
                        .HasDatabaseName("IX_AssessmentAttempt_JobSeeker_Status");

                    b.ToTable("AssessmentAttempt", (string)null);
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.AssessmentSkillScore", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("AssessmentAttemptId")
                        .HasColumnType("int");

                    b.Property<int>("CorrectAnswers")
                        .HasColumnType("int");

                    b.Property<int>("QuestionsAttempted")
                        .HasColumnType("int");

                    b.Property<decimal>("Score")
                        .HasPrecision(5, 2)
                        .HasColumnType("decimal(5,2)");

                    b.Property<int>("SkillId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("SkillId");

                    b.HasIndex("AssessmentAttemptId", "SkillId")
                        .IsUnique()
                        .HasDatabaseName("IX_AssessmentSkillScore_Attempt_Skill");

                    b.ToTable("AssessmentSkillScore", (string)null);
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Country", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("IsoCode")
                        .IsRequired()
                        .HasMaxLength(2)
                        .HasColumnType("nvarchar(2)");

                    b.Property<string>("NameAr")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("NameEn")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("PhoneCode")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<int>("SortOrder")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("IsoCode")
                        .IsUnique();

                    b.ToTable("Country", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "EG",
                            NameAr = "مصر",
                            NameEn = "Egypt",
                            PhoneCode = "+20",
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 2,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "SA",
                            NameAr = "المملكة العربية السعودية",
                            NameEn = "Saudi Arabia",
                            PhoneCode = "+966",
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 3,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "AE",
                            NameAr = "الإمارات العربية المتحدة",
                            NameEn = "United Arab Emirates",
                            PhoneCode = "+971",
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 4,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "KW",
                            NameAr = "الكويت",
                            NameEn = "Kuwait",
                            PhoneCode = "+965",
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 5,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "QA",
                            NameAr = "قطر",
                            NameEn = "Qatar",
                            PhoneCode = "+974",
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 6,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "BH",
                            NameAr = "البحرين",
                            NameEn = "Bahrain",
                            PhoneCode = "+973",
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 7,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "OM",
                            NameAr = "عمان",
                            NameEn = "Oman",
                            PhoneCode = "+968",
                            SortOrder = 7
                        },
                        new
                        {
                            Id = 8,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "JO",
                            NameAr = "الأردن",
                            NameEn = "Jordan",
                            PhoneCode = "+962",
                            SortOrder = 8
                        },
                        new
                        {
                            Id = 9,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "LB",
                            NameAr = "لبنان",
                            NameEn = "Lebanon",
                            PhoneCode = "+961",
                            SortOrder = 9
                        },
                        new
                        {
                            Id = 10,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "IQ",
                            NameAr = "العراق",
                            NameEn = "Iraq",
                            PhoneCode = "+964",
                            SortOrder = 10
                        },
                        new
                        {
                            Id = 11,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "SY",
                            NameAr = "سوريا",
                            NameEn = "Syria",
                            PhoneCode = "+963",
                            SortOrder = 11
                        },
                        new
                        {
                            Id = 12,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "YE",
                            NameAr = "اليمن",
                            NameEn = "Yemen",
                            PhoneCode = "+967",
                            SortOrder = 12
                        },
                        new
                        {
                            Id = 13,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "PS",
                            NameAr = "فلسطين",
                            NameEn = "Palestine",
                            PhoneCode = "+970",
                            SortOrder = 13
                        },
                        new
                        {
                            Id = 14,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "LY",
                            NameAr = "ليبيا",
                            NameEn = "Libya",
                            PhoneCode = "+218",
                            SortOrder = 14
                        },
                        new
                        {
                            Id = 15,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "TN",
                            NameAr = "تونس",
                            NameEn = "Tunisia",
                            PhoneCode = "+216",
                            SortOrder = 15
                        },
                        new
                        {
                            Id = 16,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "DZ",
                            NameAr = "الجزائر",
                            NameEn = "Algeria",
                            PhoneCode = "+213",
                            SortOrder = 16
                        },
                        new
                        {
                            Id = 17,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "MA",
                            NameAr = "المغرب",
                            NameEn = "Morocco",
                            PhoneCode = "+212",
                            SortOrder = 17
                        },
                        new
                        {
                            Id = 18,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "SD",
                            NameAr = "السودان",
                            NameEn = "Sudan",
                            PhoneCode = "+249",
                            SortOrder = 18
                        },
                        new
                        {
                            Id = 19,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "US",
                            NameAr = "الولايات المتحدة",
                            NameEn = "United States",
                            PhoneCode = "+1",
                            SortOrder = 100
                        },
                        new
                        {
                            Id = 20,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "GB",
                            NameAr = "المملكة المتحدة",
                            NameEn = "United Kingdom",
                            PhoneCode = "+44",
                            SortOrder = 101
                        },
                        new
                        {
                            Id = 21,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "CA",
                            NameAr = "كندا",
                            NameEn = "Canada",
                            PhoneCode = "+1",
                            SortOrder = 102
                        },
                        new
                        {
                            Id = 22,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "AU",
                            NameAr = "أستراليا",
                            NameEn = "Australia",
                            PhoneCode = "+61",
                            SortOrder = 103
                        },
                        new
                        {
                            Id = 23,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "DE",
                            NameAr = "ألمانيا",
                            NameEn = "Germany",
                            PhoneCode = "+49",
                            SortOrder = 104
                        },
                        new
                        {
                            Id = 24,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "FR",
                            NameAr = "فرنسا",
                            NameEn = "France",
                            PhoneCode = "+33",
                            SortOrder = 105
                        },
                        new
                        {
                            Id = 25,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "IT",
                            NameAr = "إيطاليا",
                            NameEn = "Italy",
                            PhoneCode = "+39",
                            SortOrder = 106
                        },
                        new
                        {
                            Id = 26,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ES",
                            NameAr = "إسبانيا",
                            NameEn = "Spain",
                            PhoneCode = "+34",
                            SortOrder = 107
                        },
                        new
                        {
                            Id = 27,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "NL",
                            NameAr = "هولندا",
                            NameEn = "Netherlands",
                            PhoneCode = "+31",
                            SortOrder = 108
                        },
                        new
                        {
                            Id = 28,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "SE",
                            NameAr = "السويد",
                            NameEn = "Sweden",
                            PhoneCode = "+46",
                            SortOrder = 109
                        },
                        new
                        {
                            Id = 29,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "NO",
                            NameAr = "النرويج",
                            NameEn = "Norway",
                            PhoneCode = "+47",
                            SortOrder = 110
                        },
                        new
                        {
                            Id = 30,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "CH",
                            NameAr = "سويسرا",
                            NameEn = "Switzerland",
                            PhoneCode = "+41",
                            SortOrder = 111
                        },
                        new
                        {
                            Id = 31,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "CN",
                            NameAr = "الصين",
                            NameEn = "China",
                            PhoneCode = "+86",
                            SortOrder = 200
                        },
                        new
                        {
                            Id = 32,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "JP",
                            NameAr = "اليابان",
                            NameEn = "Japan",
                            PhoneCode = "+81",
                            SortOrder = 201
                        },
                        new
                        {
                            Id = 33,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "IN",
                            NameAr = "الهند",
                            NameEn = "India",
                            PhoneCode = "+91",
                            SortOrder = 202
                        },
                        new
                        {
                            Id = 34,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "KR",
                            NameAr = "كوريا الجنوبية",
                            NameEn = "South Korea",
                            PhoneCode = "+82",
                            SortOrder = 203
                        },
                        new
                        {
                            Id = 35,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "SG",
                            NameAr = "سنغافورة",
                            NameEn = "Singapore",
                            PhoneCode = "+65",
                            SortOrder = 204
                        },
                        new
                        {
                            Id = 36,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "MY",
                            NameAr = "ماليزيا",
                            NameEn = "Malaysia",
                            PhoneCode = "+60",
                            SortOrder = 205
                        },
                        new
                        {
                            Id = 37,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "TH",
                            NameAr = "تايلاند",
                            NameEn = "Thailand",
                            PhoneCode = "+66",
                            SortOrder = 206
                        },
                        new
                        {
                            Id = 38,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "PH",
                            NameAr = "الفلبين",
                            NameEn = "Philippines",
                            PhoneCode = "+63",
                            SortOrder = 207
                        },
                        new
                        {
                            Id = 39,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ID",
                            NameAr = "إندونيسيا",
                            NameEn = "Indonesia",
                            PhoneCode = "+62",
                            SortOrder = 208
                        },
                        new
                        {
                            Id = 40,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "VN",
                            NameAr = "فيتنام",
                            NameEn = "Vietnam",
                            PhoneCode = "+84",
                            SortOrder = 209
                        },
                        new
                        {
                            Id = 41,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "PK",
                            NameAr = "باكستان",
                            NameEn = "Pakistan",
                            PhoneCode = "+92",
                            SortOrder = 210
                        },
                        new
                        {
                            Id = 42,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "BD",
                            NameAr = "بنغلاديش",
                            NameEn = "Bangladesh",
                            PhoneCode = "+880",
                            SortOrder = 211
                        },
                        new
                        {
                            Id = 43,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "TR",
                            NameAr = "تركيا",
                            NameEn = "Turkey",
                            PhoneCode = "+90",
                            SortOrder = 212
                        },
                        new
                        {
                            Id = 44,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "IR",
                            NameAr = "إيران",
                            NameEn = "Iran",
                            PhoneCode = "+98",
                            SortOrder = 214
                        },
                        new
                        {
                            Id = 45,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ZA",
                            NameAr = "جنوب أفريقيا",
                            NameEn = "South Africa",
                            PhoneCode = "+27",
                            SortOrder = 300
                        },
                        new
                        {
                            Id = 46,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "NG",
                            NameAr = "نيجيريا",
                            NameEn = "Nigeria",
                            PhoneCode = "+234",
                            SortOrder = 301
                        },
                        new
                        {
                            Id = 47,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "KE",
                            NameAr = "كينيا",
                            NameEn = "Kenya",
                            PhoneCode = "+254",
                            SortOrder = 302
                        },
                        new
                        {
                            Id = 48,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ET",
                            NameAr = "إثيوبيا",
                            NameEn = "Ethiopia",
                            PhoneCode = "+251",
                            SortOrder = 303
                        },
                        new
                        {
                            Id = 49,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "GH",
                            NameAr = "غانا",
                            NameEn = "Ghana",
                            PhoneCode = "+233",
                            SortOrder = 304
                        },
                        new
                        {
                            Id = 50,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "BR",
                            NameAr = "البرازيل",
                            NameEn = "Brazil",
                            PhoneCode = "+55",
                            SortOrder = 400
                        },
                        new
                        {
                            Id = 51,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "MX",
                            NameAr = "المكسيك",
                            NameEn = "Mexico",
                            PhoneCode = "+52",
                            SortOrder = 401
                        },
                        new
                        {
                            Id = 52,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "AR",
                            NameAr = "الأرجنتين",
                            NameEn = "Argentina",
                            PhoneCode = "+54",
                            SortOrder = 402
                        },
                        new
                        {
                            Id = 53,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "CL",
                            NameAr = "تشيلي",
                            NameEn = "Chile",
                            PhoneCode = "+56",
                            SortOrder = 403
                        },
                        new
                        {
                            Id = 54,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "CO",
                            NameAr = "كولومبيا",
                            NameEn = "Colombia",
                            PhoneCode = "+57",
                            SortOrder = 404
                        },
                        new
                        {
                            Id = 55,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "PL",
                            NameAr = "بولندا",
                            NameEn = "Poland",
                            PhoneCode = "+48",
                            SortOrder = 500
                        },
                        new
                        {
                            Id = 56,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "RO",
                            NameAr = "رومانيا",
                            NameEn = "Romania",
                            PhoneCode = "+40",
                            SortOrder = 501
                        },
                        new
                        {
                            Id = 57,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "GR",
                            NameAr = "اليونان",
                            NameEn = "Greece",
                            PhoneCode = "+30",
                            SortOrder = 502
                        },
                        new
                        {
                            Id = 58,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "PT",
                            NameAr = "البرتغال",
                            NameEn = "Portugal",
                            PhoneCode = "+351",
                            SortOrder = 503
                        },
                        new
                        {
                            Id = 59,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "BE",
                            NameAr = "بلجيكا",
                            NameEn = "Belgium",
                            PhoneCode = "+32",
                            SortOrder = 504
                        },
                        new
                        {
                            Id = 60,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "AT",
                            NameAr = "النمسا",
                            NameEn = "Austria",
                            PhoneCode = "+43",
                            SortOrder = 505
                        },
                        new
                        {
                            Id = 61,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "DK",
                            NameAr = "الدنمارك",
                            NameEn = "Denmark",
                            PhoneCode = "+45",
                            SortOrder = 506
                        },
                        new
                        {
                            Id = 62,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "FI",
                            NameAr = "فنلندا",
                            NameEn = "Finland",
                            PhoneCode = "+358",
                            SortOrder = 507
                        },
                        new
                        {
                            Id = 63,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "IE",
                            NameAr = "أيرلندا",
                            NameEn = "Ireland",
                            PhoneCode = "+353",
                            SortOrder = 508
                        },
                        new
                        {
                            Id = 64,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "NZ",
                            NameAr = "نيوزيلندا",
                            NameEn = "New Zealand",
                            PhoneCode = "+64",
                            SortOrder = 509
                        },
                        new
                        {
                            Id = 65,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "RU",
                            NameAr = "روسيا",
                            NameEn = "Russia",
                            PhoneCode = "+7",
                            SortOrder = 510
                        });
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Education", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Degree")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Description")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<int>("DisplayOrder")
                        .HasColumnType("int");

                    b.Property<DateTime?>("EndDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("GradeOrGPA")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Institution")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<bool>("IsCurrent")
                        .HasColumnType("bit");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<int>("JobSeekerId")
                        .HasColumnType("int");

                    b.Property<string>("Major")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("JobSeekerId", "IsDeleted")
                        .HasDatabaseName("IX_Education_JobSeekerId_IsDeleted");

                    b.ToTable("Educations", t =>
                        {
                            t.HasCheckConstraint("CK_Education_EndDateAfterStartDate", "[EndDate] IS NULL OR [EndDate] >= [StartDate]");
                        });
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.EmailVerification", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsUsed")
                        .HasColumnType("bit");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<string>("VerificationCode")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("EmailVerifications");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Experience", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("CompanyName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("DisplayOrder")
                        .HasColumnType("int");

                    b.Property<int>("EmploymentType")
                        .HasColumnType("int");

                    b.Property<DateTime?>("EndDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsCurrent")
                        .HasColumnType("bit");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<int>("JobSeekerId")
                        .HasColumnType("int");

                    b.Property<string>("JobTitle")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Responsibilities")
                        .HasMaxLength(2000)
                        .HasColumnType("nvarchar(2000)");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("JobSeekerId", "IsDeleted")
                        .HasDatabaseName("IX_Experience_JobSeekerId_IsDeleted");

                    b.ToTable("Experiences", t =>
                        {
                            t.HasCheckConstraint("CK_Experience_EndDateAfterStartDate", "[EndDate] IS NULL OR [EndDate] >= [StartDate]");
                        });
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Job", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(1200)
                        .HasColumnType("nvarchar(1200)");

                    b.Property<string>("EmploymentType")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("Location")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int>("MinYearsOfExperience")
                        .HasColumnType("int");

                    b.Property<DateTime>("PostedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("RecruiterId")
                        .HasColumnType("int");

                    b.Property<string>("Requirements")
                        .IsRequired()
                        .HasMaxLength(1200)
                        .HasColumnType("nvarchar(1200)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("RecruiterId");

                    b.ToTable("Jobs");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.JobSeeker", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("AssessmentJobTitleId")
                        .HasColumnType("int");

                    b.Property<string>("City")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int?>("CountryId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<decimal?>("CurrentAssessmentScore")
                        .HasPrecision(5, 2)
                        .HasColumnType("decimal(5,2)");

                    b.Property<int?>("FirstLanguageId")
                        .HasColumnType("int");

                    b.Property<string>("FirstLanguageProficiency")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("JobTitleId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("LastAssessmentDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("PhoneNumber")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<int?>("SecondLanguageId")
                        .HasColumnType("int");

                    b.Property<string>("SecondLanguageProficiency")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<int?>("YearsOfExperience")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CountryId");

                    b.HasIndex("FirstLanguageId");

                    b.HasIndex("JobTitleId");

                    b.HasIndex("SecondLanguageId");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("JobSeekers");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.JobSeekerSkill", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("JobSeekerId")
                        .HasColumnType("int");

                    b.Property<int>("SkillId")
                        .HasColumnType("int");

                    b.Property<string>("Source")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("Id");

                    b.HasIndex("SkillId");

                    b.HasIndex("JobSeekerId", "SkillId")
                        .IsUnique();

                    b.ToTable("JobSeekerSkills");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.JobSkill", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("JobId")
                        .HasColumnType("int");

                    b.Property<int>("SkillId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("SkillId");

                    b.HasIndex("JobId", "SkillId")
                        .IsUnique();

                    b.ToTable("JobSkills");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.JobTitle", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Category")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<int>("RoleFamily")
                        .HasColumnType("int");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id");

                    b.HasIndex("Title")
                        .IsUnique();

                    b.ToTable("JobTitle", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 2,
                            Title = "Backend Developer"
                        },
                        new
                        {
                            Id = 2,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 1,
                            Title = "Frontend Developer"
                        },
                        new
                        {
                            Id = 3,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 3,
                            Title = "Full Stack Developer"
                        },
                        new
                        {
                            Id = 4,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 4,
                            Title = "Mobile Developer"
                        },
                        new
                        {
                            Id = 5,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 4,
                            Title = "iOS Developer"
                        },
                        new
                        {
                            Id = 6,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 4,
                            Title = "Android Developer"
                        },
                        new
                        {
                            Id = 7,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 6,
                            Title = "DevOps Engineer"
                        },
                        new
                        {
                            Id = 8,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 5,
                            Title = "Data Scientist"
                        },
                        new
                        {
                            Id = 9,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 5,
                            Title = "Data Engineer"
                        },
                        new
                        {
                            Id = 10,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 5,
                            Title = "Machine Learning Engineer"
                        },
                        new
                        {
                            Id = 11,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 5,
                            Title = "AI Engineer"
                        },
                        new
                        {
                            Id = 12,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 3,
                            Title = "Software Engineer"
                        },
                        new
                        {
                            Id = 13,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 7,
                            Title = "QA Engineer"
                        },
                        new
                        {
                            Id = 14,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 7,
                            Title = "Test Automation Engineer"
                        },
                        new
                        {
                            Id = 15,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 6,
                            Title = "Cloud Engineer"
                        },
                        new
                        {
                            Id = 16,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 6,
                            Title = "Security Engineer"
                        },
                        new
                        {
                            Id = 17,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 6,
                            Title = "Cybersecurity Analyst"
                        },
                        new
                        {
                            Id = 18,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 6,
                            Title = "Network Engineer"
                        },
                        new
                        {
                            Id = 19,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 6,
                            Title = "Systems Administrator"
                        },
                        new
                        {
                            Id = 20,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 2,
                            Title = "Database Administrator"
                        },
                        new
                        {
                            Id = 21,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 3,
                            Title = "Solutions Architect"
                        },
                        new
                        {
                            Id = 22,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 3,
                            Title = "Technical Architect"
                        },
                        new
                        {
                            Id = 23,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 6,
                            Title = "Site Reliability Engineer"
                        },
                        new
                        {
                            Id = 24,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Embedded Systems Engineer"
                        },
                        new
                        {
                            Id = 25,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Game Developer"
                        },
                        new
                        {
                            Id = 26,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 2,
                            Title = "Blockchain Developer"
                        },
                        new
                        {
                            Id = 27,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "IoT Engineer"
                        },
                        new
                        {
                            Id = 28,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 5,
                            Title = "Computer Vision Engineer"
                        },
                        new
                        {
                            Id = 29,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 5,
                            Title = "NLP Engineer"
                        },
                        new
                        {
                            Id = 30,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 5,
                            Title = "Business Intelligence Analyst"
                        },
                        new
                        {
                            Id = 31,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 5,
                            Title = "Data Analyst"
                        },
                        new
                        {
                            Id = 32,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "IT Support Specialist"
                        },
                        new
                        {
                            Id = 33,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Technical Support Engineer"
                        },
                        new
                        {
                            Id = 34,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "IT Manager"
                        },
                        new
                        {
                            Id = 35,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "CTO"
                        },
                        new
                        {
                            Id = 36,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Engineering Manager"
                        },
                        new
                        {
                            Id = 37,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 3,
                            Title = "Technical Lead"
                        },
                        new
                        {
                            Id = 38,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Scrum Master"
                        },
                        new
                        {
                            Id = 39,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Product Manager"
                        },
                        new
                        {
                            Id = 40,
                            Category = "Technology",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Technical Product Manager"
                        },
                        new
                        {
                            Id = 41,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "UX Designer"
                        },
                        new
                        {
                            Id = 42,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "UI Designer"
                        },
                        new
                        {
                            Id = 43,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "UX/UI Designer"
                        },
                        new
                        {
                            Id = 44,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "Graphic Designer"
                        },
                        new
                        {
                            Id = 45,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "Web Designer"
                        },
                        new
                        {
                            Id = 46,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "Visual Designer"
                        },
                        new
                        {
                            Id = 47,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "Product Designer"
                        },
                        new
                        {
                            Id = 48,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "Interaction Designer"
                        },
                        new
                        {
                            Id = 49,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "Motion Designer"
                        },
                        new
                        {
                            Id = 50,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "3D Designer"
                        },
                        new
                        {
                            Id = 51,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "Game Designer"
                        },
                        new
                        {
                            Id = 52,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "UX Researcher"
                        },
                        new
                        {
                            Id = 53,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "Creative Director"
                        },
                        new
                        {
                            Id = 54,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "Art Director"
                        },
                        new
                        {
                            Id = 55,
                            Category = "Design",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 8,
                            Title = "Brand Designer"
                        },
                        new
                        {
                            Id = 56,
                            Category = "Marketing",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Digital Marketing Specialist"
                        },
                        new
                        {
                            Id = 57,
                            Category = "Marketing",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "SEO Specialist"
                        },
                        new
                        {
                            Id = 58,
                            Category = "Marketing",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Content Marketing Manager"
                        },
                        new
                        {
                            Id = 59,
                            Category = "Marketing",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Social Media Manager"
                        },
                        new
                        {
                            Id = 60,
                            Category = "Marketing",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Marketing Manager"
                        },
                        new
                        {
                            Id = 61,
                            Category = "Marketing",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Brand Manager"
                        },
                        new
                        {
                            Id = 62,
                            Category = "Marketing",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Growth Manager"
                        },
                        new
                        {
                            Id = 63,
                            Category = "Marketing",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Email Marketing Specialist"
                        },
                        new
                        {
                            Id = 64,
                            Category = "Marketing",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Marketing Analyst"
                        },
                        new
                        {
                            Id = 65,
                            Category = "Marketing",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Content Writer"
                        },
                        new
                        {
                            Id = 66,
                            Category = "Sales",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Sales Representative"
                        },
                        new
                        {
                            Id = 67,
                            Category = "Sales",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Account Executive"
                        },
                        new
                        {
                            Id = 68,
                            Category = "Sales",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Sales Manager"
                        },
                        new
                        {
                            Id = 69,
                            Category = "Sales",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Business Development Manager"
                        },
                        new
                        {
                            Id = 70,
                            Category = "Sales",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Customer Success Manager"
                        },
                        new
                        {
                            Id = 71,
                            Category = "Finance",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Accountant"
                        },
                        new
                        {
                            Id = 72,
                            Category = "Finance",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Financial Analyst"
                        },
                        new
                        {
                            Id = 73,
                            Category = "Finance",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Finance Manager"
                        },
                        new
                        {
                            Id = 74,
                            Category = "Finance",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "CFO"
                        },
                        new
                        {
                            Id = 75,
                            Category = "Finance",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Investment Analyst"
                        },
                        new
                        {
                            Id = 76,
                            Category = "Human Resources",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "HR Manager"
                        },
                        new
                        {
                            Id = 77,
                            Category = "Human Resources",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Recruiter"
                        },
                        new
                        {
                            Id = 78,
                            Category = "Human Resources",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Talent Acquisition Specialist"
                        },
                        new
                        {
                            Id = 79,
                            Category = "Human Resources",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "HR Business Partner"
                        },
                        new
                        {
                            Id = 80,
                            Category = "Human Resources",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "People Operations Manager"
                        },
                        new
                        {
                            Id = 81,
                            Category = "Operations",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Operations Manager"
                        },
                        new
                        {
                            Id = 82,
                            Category = "Operations",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Project Manager"
                        },
                        new
                        {
                            Id = 83,
                            Category = "Operations",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Program Manager"
                        },
                        new
                        {
                            Id = 84,
                            Category = "Operations",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Supply Chain Manager"
                        },
                        new
                        {
                            Id = 85,
                            Category = "Operations",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "Logistics Coordinator"
                        },
                        new
                        {
                            Id = 86,
                            Category = "Executive",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "CEO"
                        },
                        new
                        {
                            Id = 87,
                            Category = "Executive",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "COO"
                        },
                        new
                        {
                            Id = 88,
                            Category = "Executive",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "VP of Engineering"
                        },
                        new
                        {
                            Id = 89,
                            Category = "Executive",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "VP of Product"
                        },
                        new
                        {
                            Id = 90,
                            Category = "Executive",
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            RoleFamily = 9,
                            Title = "VP of Sales"
                        });
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Language", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("IsoCode")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<string>("NameAr")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("NameEn")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int>("SortOrder")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("IsoCode")
                        .IsUnique();

                    b.ToTable("Language", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ara",
                            NameAr = "العربية",
                            NameEn = "Arabic",
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 2,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "eng",
                            NameAr = "الإنجليزية",
                            NameEn = "English",
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 3,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "tur",
                            NameAr = "التركية",
                            NameEn = "Turkish",
                            SortOrder = 10
                        },
                        new
                        {
                            Id = 4,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "fas",
                            NameAr = "الفارسية",
                            NameEn = "Persian (Farsi)",
                            SortOrder = 11
                        },
                        new
                        {
                            Id = 5,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "heb",
                            NameAr = "العبرية",
                            NameEn = "Hebrew",
                            SortOrder = 12
                        },
                        new
                        {
                            Id = 6,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "kur",
                            NameAr = "الكردية",
                            NameEn = "Kurdish",
                            SortOrder = 13
                        },
                        new
                        {
                            Id = 7,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "urd",
                            NameAr = "الأردية",
                            NameEn = "Urdu",
                            SortOrder = 14
                        },
                        new
                        {
                            Id = 8,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "fra",
                            NameAr = "الفرنسية",
                            NameEn = "French",
                            SortOrder = 20
                        },
                        new
                        {
                            Id = 9,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "deu",
                            NameAr = "الألمانية",
                            NameEn = "German",
                            SortOrder = 21
                        },
                        new
                        {
                            Id = 10,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "spa",
                            NameAr = "الإسبانية",
                            NameEn = "Spanish",
                            SortOrder = 22
                        },
                        new
                        {
                            Id = 11,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ita",
                            NameAr = "الإيطالية",
                            NameEn = "Italian",
                            SortOrder = 23
                        },
                        new
                        {
                            Id = 12,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "por",
                            NameAr = "البرتغالية",
                            NameEn = "Portuguese",
                            SortOrder = 24
                        },
                        new
                        {
                            Id = 13,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "rus",
                            NameAr = "الروسية",
                            NameEn = "Russian",
                            SortOrder = 25
                        },
                        new
                        {
                            Id = 14,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "nld",
                            NameAr = "الهولندية",
                            NameEn = "Dutch",
                            SortOrder = 26
                        },
                        new
                        {
                            Id = 15,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "pol",
                            NameAr = "البولندية",
                            NameEn = "Polish",
                            SortOrder = 27
                        },
                        new
                        {
                            Id = 16,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ukr",
                            NameAr = "الأوكرانية",
                            NameEn = "Ukrainian",
                            SortOrder = 28
                        },
                        new
                        {
                            Id = 17,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "swe",
                            NameAr = "السويدية",
                            NameEn = "Swedish",
                            SortOrder = 29
                        },
                        new
                        {
                            Id = 18,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "nor",
                            NameAr = "النرويجية",
                            NameEn = "Norwegian",
                            SortOrder = 30
                        },
                        new
                        {
                            Id = 19,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "dan",
                            NameAr = "الدنماركية",
                            NameEn = "Danish",
                            SortOrder = 31
                        },
                        new
                        {
                            Id = 20,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "fin",
                            NameAr = "الفنلندية",
                            NameEn = "Finnish",
                            SortOrder = 32
                        },
                        new
                        {
                            Id = 21,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ell",
                            NameAr = "اليونانية",
                            NameEn = "Greek",
                            SortOrder = 33
                        },
                        new
                        {
                            Id = 22,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "zho",
                            NameAr = "الصينية (الماندرين)",
                            NameEn = "Chinese (Mandarin)",
                            SortOrder = 40
                        },
                        new
                        {
                            Id = 23,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "jpn",
                            NameAr = "اليابانية",
                            NameEn = "Japanese",
                            SortOrder = 41
                        },
                        new
                        {
                            Id = 24,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "kor",
                            NameAr = "الكورية",
                            NameEn = "Korean",
                            SortOrder = 42
                        },
                        new
                        {
                            Id = 25,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "hin",
                            NameAr = "الهندية",
                            NameEn = "Hindi",
                            SortOrder = 43
                        },
                        new
                        {
                            Id = 26,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ben",
                            NameAr = "البنغالية",
                            NameEn = "Bengali",
                            SortOrder = 44
                        },
                        new
                        {
                            Id = 27,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "vie",
                            NameAr = "الفيتنامية",
                            NameEn = "Vietnamese",
                            SortOrder = 45
                        },
                        new
                        {
                            Id = 28,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "tha",
                            NameAr = "التايلاندية",
                            NameEn = "Thai",
                            SortOrder = 46
                        },
                        new
                        {
                            Id = 29,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ind",
                            NameAr = "الإندونيسية",
                            NameEn = "Indonesian",
                            SortOrder = 47
                        },
                        new
                        {
                            Id = 30,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "msa",
                            NameAr = "الماليزية",
                            NameEn = "Malay",
                            SortOrder = 48
                        },
                        new
                        {
                            Id = 31,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "fil",
                            NameAr = "الفلبينية (تاغالوغ)",
                            NameEn = "Filipino (Tagalog)",
                            SortOrder = 49
                        },
                        new
                        {
                            Id = 32,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "swa",
                            NameAr = "السواحيلية",
                            NameEn = "Swahili",
                            SortOrder = 50
                        },
                        new
                        {
                            Id = 33,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "amh",
                            NameAr = "الأمهرية",
                            NameEn = "Amharic",
                            SortOrder = 51
                        },
                        new
                        {
                            Id = 34,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "hau",
                            NameAr = "الهوسا",
                            NameEn = "Hausa",
                            SortOrder = 52
                        },
                        new
                        {
                            Id = 35,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "yor",
                            NameAr = "اليوروبا",
                            NameEn = "Yoruba",
                            SortOrder = 53
                        },
                        new
                        {
                            Id = 36,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "zul",
                            NameAr = "الزولو",
                            NameEn = "Zulu",
                            SortOrder = 54
                        },
                        new
                        {
                            Id = 37,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ron",
                            NameAr = "الرومانية",
                            NameEn = "Romanian",
                            SortOrder = 60
                        },
                        new
                        {
                            Id = 38,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "ces",
                            NameAr = "التشيكية",
                            NameEn = "Czech",
                            SortOrder = 61
                        },
                        new
                        {
                            Id = 39,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "hun",
                            NameAr = "المجرية",
                            NameEn = "Hungarian",
                            SortOrder = 62
                        },
                        new
                        {
                            Id = 40,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "bul",
                            NameAr = "البلغارية",
                            NameEn = "Bulgarian",
                            SortOrder = 63
                        },
                        new
                        {
                            Id = 41,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "hrv",
                            NameAr = "الكرواتية",
                            NameEn = "Croatian",
                            SortOrder = 64
                        },
                        new
                        {
                            Id = 42,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "srp",
                            NameAr = "الصربية",
                            NameEn = "Serbian",
                            SortOrder = 65
                        },
                        new
                        {
                            Id = 43,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "slk",
                            NameAr = "السلوفاكية",
                            NameEn = "Slovak",
                            SortOrder = 66
                        },
                        new
                        {
                            Id = 44,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "slv",
                            NameAr = "السلوفينية",
                            NameEn = "Slovenian",
                            SortOrder = 67
                        },
                        new
                        {
                            Id = 45,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "lit",
                            NameAr = "الليتوانية",
                            NameEn = "Lithuanian",
                            SortOrder = 68
                        },
                        new
                        {
                            Id = 46,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "lav",
                            NameAr = "اللاتفية",
                            NameEn = "Latvian",
                            SortOrder = 69
                        },
                        new
                        {
                            Id = 47,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "est",
                            NameAr = "الإستونية",
                            NameEn = "Estonian",
                            SortOrder = 70
                        },
                        new
                        {
                            Id = 48,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "cat",
                            NameAr = "الكتالونية",
                            NameEn = "Catalan",
                            SortOrder = 71
                        },
                        new
                        {
                            Id = 49,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "glg",
                            NameAr = "الجاليكية",
                            NameEn = "Galician",
                            SortOrder = 72
                        },
                        new
                        {
                            Id = 50,
                            CreatedAt = new DateTime(2025, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            IsoCode = "eus",
                            NameAr = "الباسكية",
                            NameEn = "Basque",
                            SortOrder = 73
                        });
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.PasswordReset", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsUsed")
                        .HasColumnType("bit");

                    b.Property<string>("OtpCode")
                        .IsRequired()
                        .HasMaxLength(6)
                        .HasColumnType("nvarchar(6)");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("PasswordResets");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Project", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DeletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Description")
                        .HasMaxLength(1200)
                        .HasColumnType("nvarchar(1200)");

                    b.Property<int>("DisplayOrder")
                        .HasColumnType("int");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<int>("JobSeekerId")
                        .HasColumnType("int");

                    b.Property<string>("ProjectLink")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("TechnologiesUsed")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("JobSeekerId", "IsDeleted")
                        .HasDatabaseName("IX_Project_JobSeekerId_IsDeleted");

                    b.ToTable("Projects");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.QuestionBank", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("Category")
                        .HasColumnType("int");

                    b.Property<int>("CorrectAnswerIndex")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("Difficulty")
                        .HasColumnType("int");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<string>("Options")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<string>("QuestionText")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<int>("SeniorityLevel")
                        .HasColumnType("int");

                    b.Property<int?>("SkillId")
                        .HasColumnType("int");

                    b.Property<int?>("TimePerQuestion")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("SkillId");

                    b.HasIndex("Category", "Difficulty", "SeniorityLevel", "IsActive")
                        .HasDatabaseName("IX_QuestionBank_Filtering");

                    b.ToTable("QuestionBank", (string)null);
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Recommendation", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("GeneratedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("JobId")
                        .HasColumnType("int");

                    b.Property<int>("JobSeekerId")
                        .HasColumnType("int");

                    b.Property<decimal>("MatchScore")
                        .HasPrecision(5, 2)
                        .HasColumnType("decimal(5,2)");

                    b.HasKey("Id");

                    b.HasIndex("JobSeekerId");

                    b.HasIndex("JobId", "JobSeekerId")
                        .IsUnique();

                    b.ToTable("Recommendations");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Recruiter", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("CompanyDescription")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("CompanyName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("CompanySize")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Industry")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("LinkedIn")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("Location")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("LogoUrl")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<string>("Website")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.HasKey("Id");

                    b.HasIndex("UserId")
                        .IsUnique();

                    b.ToTable("Recruiters");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Resume", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<long>("FileSizeBytes")
                        .HasColumnType("bigint");

                    b.Property<string>("FileType")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("FileUrl")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<int>("JobSeekerId")
                        .HasColumnType("int");

                    b.Property<string>("ParseStatus")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateTime?>("ProcessedAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("JobSeekerId")
                        .IsUnique()
                        .HasDatabaseName("IX_Resume_JobSeekerId_Unique");

                    b.ToTable("Resumes");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Skill", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("Id");

                    b.ToTable("Skills");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.SocialAccount", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Behance")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Dribbble")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("Github")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<int>("JobSeekerId")
                        .HasColumnType("int");

                    b.Property<string>("LinkedIn")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<string>("PersonalWebsite")
                        .HasMaxLength(300)
                        .HasColumnType("nvarchar(300)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("JobSeekerId")
                        .IsUnique();

                    b.ToTable("SocialAccounts");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("AccountType")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("AuthProvider")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("FailedLoginAttempts")
                        .HasColumnType("int");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("bit");

                    b.Property<bool>("IsEmailVerified")
                        .HasColumnType("bit");

                    b.Property<DateTime?>("LastFailedLoginAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime?>("LastSuccessfulLoginAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("LockoutEnd")
                        .HasColumnType("datetime2");

                    b.Property<string>("LockoutReason")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ProfileCompletionStep")
                        .HasColumnType("int");

                    b.Property<string>("ProfilePictureUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ProviderUserId")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.ToTable("User", (string)null);
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.AssessmentAnswer", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.AssessmentAttempt", "AssessmentAttempt")
                        .WithMany("Answers")
                        .HasForeignKey("AssessmentAttemptId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("RecruitmentPlatformAPI.Models.QuestionBank", "Question")
                        .WithMany()
                        .HasForeignKey("QuestionId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("AssessmentAttempt");

                    b.Navigation("Question");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.AssessmentAttempt", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.JobSeeker", "JobSeeker")
                        .WithMany("AssessmentAttempts")
                        .HasForeignKey("JobSeekerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("RecruitmentPlatformAPI.Models.JobTitle", "JobTitle")
                        .WithMany()
                        .HasForeignKey("JobTitleId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("JobSeeker");

                    b.Navigation("JobTitle");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.AssessmentSkillScore", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.AssessmentAttempt", "AssessmentAttempt")
                        .WithMany("SkillScores")
                        .HasForeignKey("AssessmentAttemptId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("RecruitmentPlatformAPI.Models.Skill", "Skill")
                        .WithMany()
                        .HasForeignKey("SkillId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("AssessmentAttempt");

                    b.Navigation("Skill");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Education", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.JobSeeker", "JobSeeker")
                        .WithMany()
                        .HasForeignKey("JobSeekerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("JobSeeker");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.EmailVerification", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Experience", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.JobSeeker", "JobSeeker")
                        .WithMany()
                        .HasForeignKey("JobSeekerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("JobSeeker");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Job", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.Recruiter", "Recruiter")
                        .WithMany()
                        .HasForeignKey("RecruiterId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Recruiter");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.JobSeeker", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.Country", "Country")
                        .WithMany()
                        .HasForeignKey("CountryId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("RecruitmentPlatformAPI.Models.Language", "FirstLanguage")
                        .WithMany()
                        .HasForeignKey("FirstLanguageId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("RecruitmentPlatformAPI.Models.JobTitle", "JobTitle")
                        .WithMany()
                        .HasForeignKey("JobTitleId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("RecruitmentPlatformAPI.Models.Language", "SecondLanguage")
                        .WithMany()
                        .HasForeignKey("SecondLanguageId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("RecruitmentPlatformAPI.Models.User", "User")
                        .WithOne()
                        .HasForeignKey("RecruitmentPlatformAPI.Models.JobSeeker", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Country");

                    b.Navigation("FirstLanguage");

                    b.Navigation("JobTitle");

                    b.Navigation("SecondLanguage");

                    b.Navigation("User");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.JobSeekerSkill", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.JobSeeker", "JobSeeker")
                        .WithMany()
                        .HasForeignKey("JobSeekerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("RecruitmentPlatformAPI.Models.Skill", "Skill")
                        .WithMany()
                        .HasForeignKey("SkillId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("JobSeeker");

                    b.Navigation("Skill");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.JobSkill", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.Job", "Job")
                        .WithMany()
                        .HasForeignKey("JobId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("RecruitmentPlatformAPI.Models.Skill", "Skill")
                        .WithMany()
                        .HasForeignKey("SkillId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Job");

                    b.Navigation("Skill");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.PasswordReset", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Project", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.JobSeeker", "JobSeeker")
                        .WithMany()
                        .HasForeignKey("JobSeekerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("JobSeeker");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.QuestionBank", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.Skill", "Skill")
                        .WithMany()
                        .HasForeignKey("SkillId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Skill");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Recommendation", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.Job", "Job")
                        .WithMany()
                        .HasForeignKey("JobId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("RecruitmentPlatformAPI.Models.JobSeeker", "JobSeeker")
                        .WithMany()
                        .HasForeignKey("JobSeekerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Job");

                    b.Navigation("JobSeeker");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Recruiter", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.User", "User")
                        .WithOne()
                        .HasForeignKey("RecruitmentPlatformAPI.Models.Recruiter", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.Resume", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.JobSeeker", "JobSeeker")
                        .WithMany()
                        .HasForeignKey("JobSeekerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("JobSeeker");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.SocialAccount", b =>
                {
                    b.HasOne("RecruitmentPlatformAPI.Models.JobSeeker", "JobSeeker")
                        .WithOne()
                        .HasForeignKey("RecruitmentPlatformAPI.Models.SocialAccount", "JobSeekerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("JobSeeker");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.AssessmentAttempt", b =>
                {
                    b.Navigation("Answers");

                    b.Navigation("SkillScores");
                });

            modelBuilder.Entity("RecruitmentPlatformAPI.Models.JobSeeker", b =>
                {
                    b.Navigation("AssessmentAttempts");
                });
#pragma warning restore 612, 618
        }
    }
}
